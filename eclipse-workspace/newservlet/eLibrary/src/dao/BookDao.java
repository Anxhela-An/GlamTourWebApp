...
public class BookDao 
{
	public static int save(BookBean bean)
	{
		...
		try
		{
			...
			preparedStatement.setString(1, bean.getCallno());
			preparedStatement.setString(2, bean.getName());
			preparedStatement.setString(3, bean.getAuthor());
			preparedStatement.setString(4, bean.getPublisher() );
	...
	public static List< BookBean> view()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				bean.setCallno(rs.getString("id"));
	...
	public static int getIssues(String callno)
	{
		int status=0;
		...
		if(checkstatus){
			...
			try{
				...
				if(status>0){
					...
					ps2.setInt(1,getIssues(callno)+1);
